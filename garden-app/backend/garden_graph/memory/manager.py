...
try:
    ...
    SupabaseGraphRepo = None  # type: ignore
...
# Graph topology (additive layer — entities + edges between memories)
try:
    from garden_graph.memory.graph_topology import MemoryGraph
except ImportError:
...
# Anchors (user_flag=True) never decay below this floor — they are standing
# rules the character must always carry, not events that fade with time.
# Without a floor, a year-old grief anchor with weight=1.0 effectively
# becomes 0 (exp(-7.3)≈0.0007), pushing critical context out of the prompt.
ANCHOR_WEIGHT_FLOOR = 0.45
...
class MemoryRecord:
    ...
    user_flag: bool = False  # explicit anchor: never let decay drop below ANCHOR_WEIGHT_FLOOR

    # ------- helpers -------
    def effective_weight(self) -> float:
        """Return weight after applying exponential decay since last_touched.

        For user_flagged memories (anchors), the decayed weight is floored at
        ANCHOR_WEIGHT_FLOOR so the anchor remains retrievable indefinitely.
        Anchors represent standing rules ("never bring up mom unprompted")
        that must persist across the character's lifetime.
        """
        ...
        if days <= 0:
            ...
            decayed = self.weight * math.exp(-WEIGHT_DECAY * days)
        if self.user_flag:
            return max(decayed, ANCHOR_WEIGHT_FLOOR)
        return decayed
...
class MemoryManager:
    ...
    def __init__(self, memories_path: Optional[str] = None, events_path: Optional[str] = None, *, autoload: bool = False) -> None:
        ...
        if STORAGE_BACKEND == "supabase" and SupabaseMemoryRepo:
            ...
            if SupabaseGraphRepo:
                self.graph_repo = SupabaseGraphRepo()
                    ...
                    for rec_id, rec_data in data.items():
                        try:
                            self._records[rec_id] = MemoryRecord(
                                ...
                                user_flag=bool(rec_data.get("user_flag", False)),
                            )
                        except Exception as e:
        ...
        # --- Graph topology (additive layer) ---
        self._graph = None
        self._graph_path = None
        if MemoryGraph is not None:
            self._graph = MemoryGraph(
                repository=self.graph_repo,
            )
            if self.memories_path:
                self._graph_path = self.memories_path.replace(
                    "memories.json", "memory_graph.json"
                )
                if autoload and self._graph_path and os.path.exists(self._graph_path):
                    try:
                        self._graph.load_from_file(self._graph_path)
                        print(f"[MemoryManager] Graph loaded: {self._graph.entity_count} entities, {self._graph.edge_count} edges")
                    except Exception as e:
                        print(f"[MemoryManager] Graph load failed: {e}")
            # If using Supabase and no local file loaded, try loading from repo
            if autoload and self.graph_repo and self._graph.entity_count == 0:
                try:
                    self._graph.load_from_repo()
                    if self._graph.entity_count > 0:
                        print(f"[MemoryManager] Graph loaded from Supabase: {self._graph.entity_count} entities, {self._graph.edge_count} edges")
                except Exception as e:
                    print(f"[MemoryManager] Supabase graph load failed: {e}")
  ...
  "emotions": {           // intensities 0..1 for each key below
    ...
    def analyze_message(self, character_id: str, message: str, is_user_message: bool = True, llm=None) -> Optional[str]:
        ...
        if memory_command or abs(significance) > MEM_SIGNIFICANCE_THRESHOLD:
            ...
            # Graph topology extraction (additive, non-blocking)
            if self._graph is not None:
                try:
                    self._graph._character_id = character_id
                    entities, edges = self._extract_graph_data(
                        character_id, memory.id, summary[:500], llm=llm,
                    )
                    self._graph.add_memory_data(memory.id, entities, edges)
                    if self._graph_path:
                        self._graph.save_to_file(self._graph_path)
                except Exception as err:
                    print(f"[MemoryManager] Graph extraction error: {err}")
    ...
    _GRAPH_EXTRACT_PROMPT = """Extract entities and relationships from this memory event about a user.

Event: "{text}"

Existing memories for context (to find relationships):
{existing}

Return ONLY valid JSON:
{{
  "entities": [
    {{"name": "lowercase_name", "entity_type": "person|place|concept|organization", "aliases": ["alias1"]}}
  ],
  "edges": [
    {{"related_memory_snippet": "exact phrase from an existing memory", "edge_type": "cause_of|temporal_before|amplifies|contradicts", "confidence": 0.8}}
  ]
}}

Rules:
- Entity names must be lowercase, single canonical form
- Only include edges with confidence >= 0.5
- Edge types: cause_of (this event caused that), temporal_before (this happened before that), amplifies (this intensifies that), contradicts (this conflicts with that)
- related_memory_snippet must be a recognizable substring from the existing memories listed above
- If no clear relationships, return empty arrays"""
    ...
    def _extract_graph_data(
        ...
        """Extract entities and edges from a newly created memory.

        Returns (entities_list, edges_list) where edges reference existing memory IDs.
        Uses LLM for extraction; falls back to empty lists on failure.
        """
        if not llm:
            return [], []

        # Build context: short summaries of existing memories for edge resolution
        active = self.all_active(character_id)
        existing_lines = []
        for r in active[-30:]:  # last 30 to keep prompt short
            if r.id == memory_id:
                continue
            existing_lines.append(f"[{r.id[:8]}] {r.event_text[:120]}")
        existing_ctx = "\n".join(existing_lines) if existing_lines else "(none)"

        prompt = self._GRAPH_EXTRACT_PROMPT.format(
            text=event_text, existing=existing_ctx,
        )

        try:
            from langchain_core.messages import SystemMessage, HumanMessage
            response = llm.invoke([
                SystemMessage(content="You are an entity/relationship extractor. Return only valid JSON."),
            ...
            print(f"[MemoryManager] Graph LLM extraction failed: {e}")
            return [], []

        entities = data.get("entities", [])

        # Resolve edge snippets to actual memory IDs
        resolved_edges = []
        for edge_data in data.get("edges", []):
            snippet = edge_data.get("related_memory_snippet", "")
            if not snippet:
                continue
            ...
            for r in active:
                if r.id == memory_id:
                    continue
                if snippet_lower in r.event_text.lower():
                    # Exact substring match
                    best_match = r.id
                    best_score = 1.0
                    break
                # Fallback: token overlap
                s_tokens = set(snippet_lower.split())
                r_tokens = set(r.event_text.lower().split())
                overlap = len(s_tokens & r_tokens) / max(1, len(s_tokens))
            ...
            if best_match:
                resolved_edges.append({
                    ...
                    "edge_type": edge_data.get("edge_type", "amplifies"),
                    "confidence": float(edge_data.get("confidence", 0.5)),
                })

        return entities, resolved_edges
    ...
    # ---- empathic salience retrieval -----------------------------------
    # Standing-rule anchors (user_flag=True) ALWAYS get a slot when their
    # topic touches the query — that's the central insight separating
    # empathic memory from semantic search. The companion needs to know
    # "never bring up mom unprompted" BEFORE it sees any mom-content.
    #
    # salient_memories dispatches to one of four modes:
    #   1. cold-open texture (no content tokens, neutral valence) → mix
    #      of dominant joy + standing anchor + active hidden weight
    #   2. recency mode ("what's recent" / "what happened lately") →
    #      recency-decayed score with sentiment magnitude tiebreak
    #   3. valence mode (no content but emotional valence detected,
    #      e.g. "what's weighing") → events of matching sentiment
    #   4. content mode (real keyword query) → topic-cluster expanded
    #      keyword overlap + anchor warning boost + recency
    #
    # Why dispatch instead of one big formula: each empathic intent
    # needs a different MIX. A cold open wants texture (joy + anchor +
    # weight), a recency query wants recent things, a "weighing" query
    # wants the active heavy items. One scalar can't express all four.

    _SALIENCE_STOPWORDS: frozenset = frozenset({
        "a", "an", "the", "is", "are", "was", "were", "be", "been", "being",
        "of", "to", "in", "on", "for", "with", "at", "by", "from", "up",
        "down", "and", "or", "but", "not", "no", "as", "if", "then", "than",
        "so", "such", "this", "that", "these", "those", "it", "its", "i",
        "me", "my", "you", "your", "we", "our", "they", "them", "their",
        "he", "she", "his", "her", "him", "what", "which", "who", "whom",
        "whose", "where", "when", "why", "how", "do", "does", "did", "have",
        "has", "had", "should", "would", "could", "can", "will", "may",
        "might", "about", "into", "out", "over", "under", "again", "more",
        "most", "some", "any", "all", "much", "very", "just", "also",
        "tell", "know", "bring", "next", "context", "currently", "right",
        "now", "today", "ask", "asking", "going", "happen", "happens",
        "feeling", "feel", "feels", "want", "wants", "wanted",
        "important", "matter", "matters", "thing", "things", "stuff",
        "say", "says", "said", "see", "seen", "seeing", "make", "made",
        "get", "got", "give", "given", "let", "lets", "take", "taken",
        "let", "lets", "been", "being", "yes", "okay", "ok",
    })

    # Recency-intent words: when present, dispatch to recency mode
    _SALIENCE_RECENCY_HINTS: frozenset = frozenset({
        "recent", "recently", "lately", "today", "yesterday", "week",
        "weeks", "happened", "happen", "happens", "lately", "now",
        "new", "latest", "current",
    })

    # Negative-valence query hints (asking about heavy / weighing content)
    _SALIENCE_NEG_HINT: frozenset = frozenset({
        "weighing", "weighs", "weight", "heavy", "hard", "difficult",
        "struggling", "struggle", "sad", "grief", "grieving", "loss",
        "lost", "hurt", "pain", "stress", "stressed", "worried",
        "concerned", "upset", "depressed", "anxious", "anxiety", "afraid",
        "fear", "lonely", "tired", "exhausted", "overwhelmed", "burden",
        "burdens", "trouble", "troubled", "wound", "wounded", "carry",
        "carrying", "weighing", "emotionally", "rough", "tough",
    })
    _SALIENCE_POS_HINT: frozenset = frozenset({
        "joy", "joyful", "happy", "best", "good", "love", "loved", "loving",
        "proud", "excited", "celebrating", "celebrate", "celebration",
        "victory", "win", "winning", "success", "successful", "grateful",
        "delighted", "pleased", "thriving", "wonderful",
    })

    # Light kinship/synonym map. The colloquial "mom" must reach the
    # canonical anchor text "mother". Kept small on purpose.
    _SALIENCE_SYNONYMS: dict = {
        ...
        "fiance": {"engaged", "engagement", "fiancee"},
        "fiancee": {"engaged", "engagement", "fiance"},
        "wedding": {"marriage", "married"},
        "marriage": {"wedding", "married"},
        "job": {"work", "career", "client", "account"},
        "work": {"job", "career", "client", "account"},
        "sad": {"grief", "grieving", "loss", "lost", "hurt", "pain"},
        "happy": {"joy", "joyful", "celebration", "celebrate"},
        "anxious": {"anxiety", "worried", "stress"},
        "dog": {"puppy", "retriever"},
    }

    # Topic clusters: when query mentions a keyword, expand to the
    # whole cluster. This is the main way Garden bridges colloquial
    # questions ("about his family") to specific anchor text ("his
    # estranged father David").
    _SALIENCE_TOPIC_CLUSTERS: dict = {
        "family": {
            "mother", "father", "parent", "parents", "sibling", "siblings",
            "brother", "sister", "son", "daughter", "kid", "kids", "child",
            "children", "spouse", "husband", "wife", "dad", "mom", "papa",
            "mama", "fiance", "fiancee", "engaged", "engagement", "wedding",
            "married", "marriage", "relatives", "uncle", "aunt", "grandma",
            "grandpa", "grandmother", "grandfather", "estranged",
        },
        "work": {
            "job", "career", "client", "account", "boss", "colleague",
            "company", "team", "project", "office", "workplace", "employer",
            "employee", "coworker", "coworkers", "layoff", "layoffs",
            "fired", "promotion",
        },
        "health": {
            "medication", "medicine", "doctor", "therapy", "hospital",
            "clinic", "diagnosis", "anxiety", "depression", "pain",
            "illness", "injury", "surgery", "sertraline", "prescription",
            "epipen", "allergic", "allergy",
        },
        "pet": {"dog", "cat", "puppy", "kitten", "retriever"},
        "friends": {"friend", "buddy", "buddies", "pal", "pals"},
        "love": {
            "romance", "romantic", "boyfriend", "girlfriend", "partner",
            "love", "loved", "loving", "engaged", "engagement", "wedding",
        },
    }

    def _salience_tokenize(self, text: str, dynamic_stop: Optional[set] = None) -> set:
        if not text:
            return set()
        tokens = re.findall(r"[\w']+", text.lower())
        # Strip possessives: "alex's" → "alex" so dynamic stopwords catch them
        tokens = [t[:-2] if t.endswith("'s") else t for t in tokens]
        out = {t for t in tokens if len(t) > 2 and t not in self._SALIENCE_STOPWORDS}
        if dynamic_stop:
            out -= dynamic_stop
        return out

    def _salience_expand(self, tokens: set) -> set:
        """Expand tokens via synonyms AND topic clusters."""
        expanded = set(tokens)
        for t in list(tokens):
            if t in self._SALIENCE_SYNONYMS:
                expanded |= self._SALIENCE_SYNONYMS[t]
            if t in self._SALIENCE_TOPIC_CLUSTERS:
                expanded |= self._SALIENCE_TOPIC_CLUSTERS[t]
        return expanded

    def _salience_query_valence(self, raw_lower: str) -> int:
        if not raw_lower:
            return 0
        tokens = set(re.findall(r"[\w']+", raw_lower))
        neg = len(tokens & self._SALIENCE_NEG_HINT)
        pos = len(tokens & self._SALIENCE_POS_HINT)
        if neg > pos:
            return -1
        if pos > neg:
            return 1
    ...
    def _salience_is_recency(self, raw_lower: str) -> bool:
        if not raw_lower:
            return False
        tokens = set(re.findall(r"[\w']+", raw_lower))
        return bool(tokens & self._SALIENCE_RECENCY_HINTS)

    def _salience_dynamic_stopwords(self, active: List[MemoryRecord]) -> set:
        """Tokens appearing in >50% of active events are noise (e.g. the
        user's name). Filter them out so query relevance isn't dominated
        by trivial matches like 'alex' present in every event.
        """
        n = len(active)
        if n < 8:
            return set()
        df: Dict[str, int] = {}
        for rec in active:
            for t in self._salience_tokenize(rec.event_text):
                df[t] = df.get(t, 0) + 1
        threshold = max(2, int(n * 0.5))
        return {t for t, c in df.items() if c >= threshold}

    def _cold_open_texture(
        self, active: List[MemoryRecord], k: int
    ) -> List[MemoryRecord]:
        """Cold-open: dominant joy + standing anchor + active hidden weight.

        This is what an empathic companion needs at the start of a session
        when the user has not yet named a topic — the texture of the person
        right now: what's bright, what's carried, what's quiet but heavy.
        """
        positives = sorted(
            [r for r in active if r.sentiment > 0],
            key=lambda r: r.effective_weight(),
            reverse=True,
        )
        anchors = sorted(
            [r for r in active if r.user_flag],
            key=lambda r: r.effective_weight(),
            reverse=True,
        )
        # Heavy = negative non-anchors (anchors have their own slot)
        heavy = sorted(
            [r for r in active if r.sentiment < 0 and not r.user_flag],
            key=lambda r: r.effective_weight(),
            reverse=True,
        )

        result: List[MemoryRecord] = []
        # Slot 1: dominant joy (often an anchor itself like an engagement)
        if positives:
            result.append(positives[0])
        # Slot 2: top anchor not already chosen
        for a in anchors:
            if a not in result:
                result.append(a)
                break
        # Slot 3: top heavy non-anchor — the unspoken weight
        for h in heavy:
            if h not in result:
                result.append(h)
                break
        # Fallback fill if any bucket was empty
        for r in sorted(active, key=lambda r: r.effective_weight(), reverse=True):
            if r not in result and len(result) < k:
                result.append(r)
        return result[:k]
    ...
    def _recency_query(
        ...
        """For 'what's recent' / 'what happened lately' queries.

        Recency DOMINATES.  Sentiment magnitude and anchor status are
        multiplicative tie-breakers — they nudge memories of similar age
        but can never override a large age gap.  This prevents a 30-day-old
        high-sentiment anchor from outranking a 12-day-old emotional spike.
        """
        now = datetime.now(timezone.utc)

        def score(r: MemoryRecord) -> float:
            days = (now - r.last_touched).total_seconds() / 86400.0
            if days < 0:
                days = 0
            # Half-life ~14 days — strong recency bias
            recency = math.exp(-0.05 * days)
            # Multiplicative: sentiment breaks ties, never overrides age
            sent_mult = 1.0 + abs(r.sentiment) * 0.15   # 1.0 – 1.3
            anchor_mult = 1.05 if r.user_flag else 1.0
            return recency * sent_mult * anchor_mult

        return sorted(active, key=score, reverse=True)[:k]

    def _valence_query(
        self, active: List[MemoryRecord], valence: int, k: int
    ) -> List[MemoryRecord]:
        """For 'what's weighing on Alex' / 'what is Alex happy about' queries.

        Filters by sentiment alignment, then ranks by raw decayed weight
        (no anchor floor — we want the ACTIVE heavy/joyful events, not
        the standing rules). Always ensures one matching anchor surfaces
        as the carried context.
        """
        now = datetime.now(timezone.utc)
        aligned = [
            r for r in active
            if (valence < 0 and r.sentiment < 0) or (valence > 0 and r.sentiment > 0)
        ]
        if not aligned:
            return self._cold_open_texture(active, k)

        def score(r: MemoryRecord) -> float:
            days = (now - r.last_touched).total_seconds() / 86400.0
            if days < 0:
                days = 0
            raw = r.weight * math.exp(-WEIGHT_DECAY * days)
            mag = abs(r.sentiment) * 0.25
            return raw + mag

        ranked = sorted(aligned, key=score, reverse=True)
        result = ranked[:k]

        # Ensure at least one anchor surfaces — the standing weight matters
        has_anchor = any(r.user_flag for r in result)
        if not has_anchor:
            top_anchor = next((r for r in ranked if r.user_flag), None)
            if top_anchor:
                result = result[:k - 1] + [top_anchor]
        return result[:k]
    ...
    def _content_query(
        ...
        """For real keyword queries: blend topic match + anchor warning + weight."""
        n_q = max(1, len(q_tokens))

        def relevance(rec: MemoryRecord) -> float:
            text_tokens = self._salience_tokenize(rec.event_text, dynamic_stop)
            if not text_tokens:
                return 0.0
            overlap = len(q_expanded & text_tokens)
            return overlap / n_q

        def score(rec: MemoryRecord) -> float:
            base = rec.effective_weight()
            rel = relevance(rec)
            # Anchors carry standing presence
            if rec.user_flag:
                base += 0.15
            # Strong content match
            base += 1.6 * rel
            # Anchors with topic match: NEGATIVE-sentiment anchors are
            # warnings (highest priority — companion must avoid landmines).
            # Positive anchors get a smaller boost (they're nice-to-know).
            if rec.user_flag and rel > 0:
                if rec.sentiment < 0:
                    base += 0.7
                else:
                    base += 0.2
            # Valence alignment when query has emotional undertone
            if q_valence != 0 and rec.sentiment != 0:
                if (q_valence < 0 and rec.sentiment < 0) or (q_valence > 0 and rec.sentiment > 0):
                    base += 0.3 * abs(rec.sentiment)
            return base

        ranked = sorted(active, key=score, reverse=True)
        result = ranked[:k]

        # Guarantee one matching non-anchor surfaces if we're crowded
        # with anchors. The non-anchor is often the SAFE thing the
        # companion can lead with (e.g. "ethan visited last weekend").
        if any(relevance(r) > 0 for r in active):
            has_matching_non_anchor = any(
                (not r.user_flag) and relevance(r) > 0 for r in result
            )
            if not has_matching_non_anchor:
                top_match = next(
                    (r for r in ranked if not r.user_flag and relevance(r) > 0),
                    None,
                )
                if top_match:
                    result = result[:k - 1] + [top_match]
        return result[:k]

    def salient_memories(
        self,
        character_id: str,
        query: Optional[str] = None,
        k: int = 3,
    ) -> List[MemoryRecord]:
        """Salience-aware retrieval for empathic AI companions.

        Dispatches to one of four modes based on the query:
          1. **cold-open texture** — no query, or query collapses to no
             content tokens. Returns dominant joy + top anchor + active
             hidden weight, the texture an empathic companion needs at
             the start of a session.
          2. **recency mode** — query mentions "recent / lately / today /
             this week". Returns events sorted by recency × sentiment
             magnitude. Anchors get a small carry-bonus.
          3. **valence mode** — query has no content tokens but contains
             emotional valence words ("weighing", "joy"). Returns events
             of matching sentiment, ranked by raw decayed weight.
          4. **content mode** — query has real content tokens. Expands
             via synonyms + topic clusters, scores by keyword overlap +
             anchor warning boost + recency. Guarantees one matching
             non-anchor surfaces alongside anchor warnings.

        The dispatch matters: a single scalar score cannot express the
        difference between "give me texture" and "give me what's heavy"
        and "give me what's recent". The four modes are the empathic
        intents Garden was built to understand.
        """
        active = self.all_active(character_id)
        if not active:
            return []
        ...
        dynamic_stop = self._salience_dynamic_stopwords(active)
        q_valence = self._salience_query_valence(raw_lower)
        is_recency = self._salience_is_recency(raw_lower)

        if not query or not query.strip():
            return self._cold_open_texture(active, k)

        q_tokens = self._salience_tokenize(query, dynamic_stop)
        q_expanded = self._salience_expand(q_tokens)

        if is_recency:
            return self._recency_query(active, k)
        if not q_expanded:
            if q_valence != 0:
                return self._valence_query(active, q_valence, k)
            return self._cold_open_texture(active, k)
        return self._content_query(active, q_tokens, q_expanded, q_valence, dynamic_stop, k)
    ...
    def salient_memories_with_graph(
        self,
        character_id: str,
        query: Optional[str] = None,
        k: int = 3,
        ...
        """Salient memories + query-filtered graph-expanded related memories.

        Returns up to k + expand records.  The first k are from
        salient_memories() (unchanged).  The remaining are graph-discovered
        additions filtered by a relevance gate that prevents topic drift.
        """
        core = self.salient_memories(character_id, query, k)
        if not self._graph or not core:
            return core

        active = self.all_active(character_id)
        seed_ids = [r.id for r in core]
        active_ids = {r.id for r in active}

        # Over-fetch candidates — the relevance gate will filter them down
        expanded = self._graph.expand_related(
            seed_ids, active_ids, max_hops=1, max_expand=expand * 3,
        )

        # Dynamic stopwords from full active set — filters ubiquitous tokens
        # like character names that would create false relevance matches
        dynamic_stop = self._salience_dynamic_stopwords(active)
        extra = self._filter_graph_expansions(expanded, core, query, dynamic_stop)
        return core + extra[:expand]

    def _filter_graph_expansions(
        self,
        expanded: list,
        core: List[MemoryRecord],
        query: Optional[str],
        dynamic_stop: Optional[set] = None,
    ) -> List[MemoryRecord]:
        """Query-aware relevance gate for graph-expanded memories.

        Three gates that fix the topology-without-intent problem:

        1. **Anchor safety** — anchors (user_flag=True) are landmines that
           must surface only through core scoring with its safety logic.
           Graph expansion must never bypass that.

        2. **Recency block** — recency queries want *recent* events.  Graph
           expansion walks topology, which is time-agnostic.  Adding old
           memories to a recency result is pure noise.

        3. **Keyword relevance** — the expanded memory's text must share at
           least one content token (after synonym/topic expansion) with the
           query.  Uses dynamic stopwords to filter ubiquitous tokens like
           character names that would create false relevance matches.

        Cold-open (no query) skips gates 2+3 — graph adds useful texture.
        """
        if not expanded:
            return []
        ...
        # Gate 2: recency queries — graph expansion adds old noise
        if query and self._salience_is_recency(raw_lower):
            return []

        # Build expanded query token set with dynamic stopwords
        q_tokens: set = set()
        if query and query.strip():
            q_tokens = self._salience_tokenize(query, dynamic_stop)
            q_tokens |= self._salience_expand(q_tokens)

        # Detect query valence for sentiment-aligned expansion
        q_valence = self._salience_query_valence(raw_lower) if query else 0

        core_ids = {r.id for r in core}
        filtered: List[MemoryRecord] = []

        for mem_id, _reason in expanded:
            rec = self.get(mem_id)
            if not rec or rec.id in core_ids:
                continue

            # Gate 1: anchor safety — never surface anchors via graph
            if rec.user_flag:
                continue

            # Gate 3: keyword relevance OR sentiment alignment
            # Keyword match prevents topical drift (grief→engagement).
            # Sentiment alignment lets emotionally congruent memories through
            # on valence queries ("what weighs" → negative memories pass).
            if q_tokens:
                mem_tokens = self._salience_tokenize(rec.event_text, dynamic_stop)
                mem_tokens |= self._salience_expand(mem_tokens)
                keyword_match = bool(q_tokens & mem_tokens)

                sentiment_match = (
                    (q_valence < 0 and rec.sentiment < 0) or
                    (q_valence > 0 and rec.sentiment > 0)
                )

                if not keyword_match and not sentiment_match:
                    continue
    ...
    def prompt_segment(self, character_id: str, k: int = 3) -> str:
        ...
        if self._graph is not None:
            recs = self.salient_memories_with_graph(character_id, query=None, k=k, expand=2)
            core_recs = recs[:k]
            graph_recs = recs[k:]
        else:
            core_recs = self.top_k(character_id, k)
            graph_recs = []
        ...
        if graph_recs:
            segments.append("Related context (connected memories):")
            for r in graph_recs:
                segments.append(f"• [{r.event_text}] (w={r.effective_weight():.2f})")
    ...
    def save_to_file(self, filepath: str) -> bool:
        ...
        try:
            ...
            # Persist graph topology alongside memories
            if self._graph and self._graph_path:
                try:
                    self._graph.save_to_file(self._graph_path)
                except Exception as ge:
                    print(f"[MemoryManager] Graph save error: {ge}")
