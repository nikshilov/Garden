...
final class ChatViewModel: ObservableObject {
    ...
    @Published var totalPromptTokens: Int = 0
    @Published var totalCompletionTokens: Int = 0
    ...
    func meta(for exyteId: String) -> (cost: Double?, time: TimeInterval?, model: String?, promptTokens: Int?, completionTokens: Int?)? {
        guard let chatMsg = messages.first(where: { $0.id.uuidString == exyteId }) else { return nil }
        return (chatMsg.costUSD, chatMsg.responseTime, chatMsg.model, chatMsg.promptTokens, chatMsg.completionTokens)
    }
    ...
    private static func parseMultiSpeakerResponse(_ text: String, totalCost: Double?, totalDuration: TimeInterval?,
                                                    model: String? = nil, promptTokens: Int? = nil, completionTokens: Int? = nil) -> [ChatMessage] {
        let pattern = #"\*\*(.*?)\*\*:\s*(.*?(?=\n\*\*|$))"#
    ...
    func send(text: String) {
        ...
        Task { @MainActor in
            ...
            do {
                ...
                let botMessages = ChatViewModel.parseMultiSpeakerResponse(
                    response.text, totalCost: response.cost_total_usd, totalDuration: duration,
                    model: response.model, promptTokens: response.prompt_tokens, completionTokens: response.completion_tokens
                )
                ...
                totalPromptTokens += response.prompt_tokens ?? 0
                totalCompletionTokens += response.completion_tokens ?? 0
