...
struct SettingsView: View {
    ...
    private var displaySection: some View {
        Section {
            Toggle("Show cost & time per message", isOn: $showCostMeta)
            Toggle("Debug panel", isOn: $debugMode)
        } header: {
            Text("Display")
        } footer: {
            Text("Debug panel shows model, tokens, cost, and session info. You can also triple-tap the chat header to toggle it.")
        }
    }
